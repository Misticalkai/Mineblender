...
func _enter_tree() -> void:
	...
	if console_history_file:
		while not console_history_file.eof_reached():
...
func _generate_hardware_id() -> String:
	# Check for existing Hardware ID in file
	var file := FileAccess.open("user://hardware_id.txt", FileAccess.READ)
	if file:
		var id = file.get_line().strip_edges()
		file.close()
		if id.length() == 36:  # Validate UUID format
			return id
	...
	# Store UUID in file
	file = FileAccess.open("user://hardware_id.txt", FileAccess.WRITE)
	if file:
...
func print_welcome_messages() -> void:
	welcome_messages = "[color=light_green]Welcome to Mineblender Console![/color]\n"
	welcome_messages += "[b][color=white]Info:[/color][/b]\n"
	welcome_messages += "[color=white]Version: 1.0.0.1 Dev 1[/color]\n"
	welcome_messages += "[color=white]Version Name: 1MB101[/color]\n"
	welcome_messages += "[color=white]Hardware ID: " + _generate_hardware_id() + "[/color]\n"
	welcome_messages += "[color=white]CPU: " + OS.get_processor_name() + "[/color]\n"
	welcome_messages += "[color=white]GPU: " + RenderingServer.get_video_adapter_name() + "[/color]\n"
	var is_rtx = RenderingServer.get_video_adapter_vendor().to_lower().contains("nvidia") and RenderingServer.get_video_adapter_name().to_lower().contains("rtx")
	welcome_messages += "[color=" + ("green" if is_rtx else "red") + "]Ray Tracing: " + ("RTX Ready GPU Enabled" if is_rtx else "Incompatible GPU") + "[/color]\n"
	rich_label.append_text(welcome_messages)
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey:
		if event.get_physical_keycode_with_modifiers() == KEY_F2:
			if event.pressed:
				toggle_console()
	...
	if suggesting:
		...
		if " " in line_edit.text:
			...
			for command in console_commands:
				if not console_commands[command].hidden:
					sorted_commands.append(str(command))
			sorted_commands.sort()
			sorted_commands.reverse()
			var prev_index := 0
			for command in sorted_commands:
				if not line_edit.text or command.contains(line_edit.text):
...
func help() -> void:
	...
	rich_label.append_text("        [color=light_green]calc[/color]: Calculates a given expression\n")
	rich_label.append_text("        [color=light_green]clear[/color]: Clears the registry view\n")
	rich_label.append_text("        [color=light_green]commands[/color]: Shows a reduced list of all the currently registered commands\n")
	rich_label.append_text("        [color=light_green]commands_list[/color]: Shows a detailed list of all the currently registered commands\n")
	rich_label.append_text("        [color=light_green]delete_history[/color]: Deletes the commands history\n")
	rich_label.append_text("        [color=light_green]echo[/color]: Prints a given string to the console\n")
	rich_label.append_text("        [color=light_green]echo_error[/color]: Prints a given string as an error to the console\n")
	rich_label.append_text("        [color=light_green]echo_info[/color]: Prints a given string as info to the console\n")
	rich_label.append_text("        [color=light_green]echo_warning[/color]: Prints a given string as warning to the console\n")
	rich_label.append_text("        [color=light_green]pause[/color]: Pauses node processing\n")
	rich_label.append_text("        [color=light_green]unpause[/color]: Unpauses node processing\n")
	rich_label.append_text("        [color=light_green]quit[/color]: Quits the game\n")
	...
	rich_label.append_text("        [color=light_blue]PageUp[/color] and [color=light_blue]PageDown[/color] to scroll registry\n")
...
func add_input_history(text: String) -> void:
	if not console_history.size() or text != console_history.back():
...
func set_enable_on_release_build(enable: bool):
	...
	if not enable_on_release_build:
